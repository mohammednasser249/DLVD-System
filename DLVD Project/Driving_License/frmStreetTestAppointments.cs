...
namespace DLVD_Project.Driving_License
{
    ...
    {
        int LicnseId;
        public frmStreetTestAppointments(int licnseId)
        {
            InitializeComponent();
            LicnseId = licnseId;    
        }

        private void _GetAllStreetTest(int ID)
        {

            DataTable dt = clsTestAppointmentsBL.GetStreetTestAppointemnts(ID);
     ...
     dataGridView1.CurrentRow.Cells[0].Value != DBNull.Value)
            ...
            if (check && check2)
            {
                ...
                // change 
                frmMakeStreetTest frm = new frmMakeStreetTest(-1, LicnseId);
                frm.ShowDialog();
                _GetAllStreetTest(LicnseId);
     ...
     dataGridView1.CurrentRow.Cells[0].Value != DBNull.Value)
            ...
            if (!check)
            {
                frmMakeStreetTest frm = new frmMakeStreetTest((int)dataGridView1.CurrentRow.Cells[0].Value, LicnseId);
                frm.ShowDialog();
                _GetAllStreetTest(LicnseId);
            }
            else
     ...
     dataGridView1.CurrentRow.Cells[0].Value != DBNull.Value)
            ...
            if (check2)
            {
                frmTakeStreetTest frm = new frmTakeStreetTest((int)dataGridView1.CurrentRow.Cells[0].Value);
                frm.ShowDialog();
                _GetAllStreetTest(LicnseId);
            }
            else
